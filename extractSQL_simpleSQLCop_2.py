                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    for line_number, line in enumerate(file, start=1):  # 行番号をカウント
                        try:
                                ...
                                # results.append([filenasme, dirpath, filename,file_content])
                            ...
                            # 一致があれば結果に追加
                                ...
                                sql = line
                                from_match = re.search(r'([\w.]+)\s*=\s*.*?\s+FROM\s+([\w.]+)(?:\s+AS\s+(\w+)|\s+(\w+))?', sql, re.IGNORECASE)
                                file_name = from_match.group(1) if from_match else None
                                from_table = from_match.group(2) if from_match else None
                                from_alias = from_match.group(3) if from_match and from_match.group(4) else ""
                                ...
                                for join in join_matches:
                                    ...
                                    results.append([filename,dirpath,file_name,from_table,from_alias,join_table, join_alias,left_column,right_column])
